...
def subjectchoose(text_to_speech):
    def calculate_attendance():
        ...
        filenames = [file for file in os.listdir(
            f"Attendance\\{Subject}") if os.path.isfile(f'Attendance\\{Subject}\\{file}')]
        # filenames = glob(
        #     f"Attendance\\{Subject}\\?.csv"
        # )
        print(filenames)
        df = [pd.read_csv(f"Attendance\\{Subject}\\{f}") for f in filenames]
        newdf = df[0]
    ...
    titl = tk.Label(subject, bg="gray", relief=RIDGE,
                    bd=10, font=("arial", 30))
    titl.pack(fill=X)
